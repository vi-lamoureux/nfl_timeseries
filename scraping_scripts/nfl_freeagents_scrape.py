...
for link_suffix in link_suffixes:
    ...
    while not success and len(list_proxy) > 0:
        ...
        try:
            ...
            dob = clean(full_info.find_all("p")[3])
            college = full_info.find_all("p")[4].find("a").text.strip()

            # Write row of variables to csv file
            with open('pfref_data_fa.csv', 'a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([name, position, ht_wt, dob, college])
